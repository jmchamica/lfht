...
void *pt_verify(void *entry_point)
{
	int tid = ((intptr_t)entry_point);
	...
	for(int i = 0; i < nodes; i++) {
		...
		if (ratios.cr < operations.rr * nodes && rng < ratios.lr) {
			...
			value = rng * GOLD_RATIO;
			uintptr_t s = (uintptr_t)lfht_search(head, value, tid);
			if(s != 0) {
				printf("(R) Expected %d but got %lu, instead\n", 0, s);
			}
			continue;
		}
		...
		if (ratios.ci < operations.ri * nodes && rng < ratios.li) {
			...
			uintptr_t s = (uintptr_t)lfht_search(head, value, tid);
			if(s != value) {
				printf("(I) Expected %lu but got %lu, instead\n", value, s);
			}
			continue;
		}
...
int main(int argc, char **argv)
{
	...
	int cores_per_node = nproc / NUMA_NODES;
	nproc = ((int)((n_threads-1) / cores_per_node) + 1) * cores_per_node;
	if (nproc > processors) {
		nproc = processors;
	}
	...
	// // Optional
	// printf("[Stage 5] Verify test authenticity...\n");
	// for(int i = 0; i < n_threads; i++) {
	// 	lfht_init_thread(head, i);
	// }
	// for(int i = 0; i < n_threads; i++) {
	// 	pthread_create(&threads[i], NULL, pt_verify, (void*)(intptr_t)i);
	// }
	// for(int i = 0; i < n_threads; i++) {
	// 	pthread_join(threads[i], NULL);
	// }
